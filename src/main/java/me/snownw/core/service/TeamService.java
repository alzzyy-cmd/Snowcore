...
/** SnowNW takım sistemi: Ultimate tarzı takım, davet, takım sohbeti, ev ve dost ateşi. */
public final class TeamService {
    public static final class Team {
        ...
        public String id(){return id;} public String name(){return name;} public UUID leader(){return leader;} public Set<UUID> members(){return members;} public Location home(){return home;} public boolean friendlyFire(){return friendlyFire;}
    ...
    public void load(){
        ...
        for(String id:root.getKeys(false)){
            ...
            try {
                ...
                Location home=null; String world=s.getString("home.world"); if(world!=null&&Bukkit.getWorld(world)!=null) home=new Location(Bukkit.getWorld(world),s.getDouble("home.x"),s.getDouble("home.y"),s.getDouble("home.z"),(float)s.getDouble("home.yaw"),(float)s.getDouble("home.pitch"));
    ...
    public Team get(Player p){return p==null?null:teams.get(byPlayer.get(p.getUniqueId()));}
    ...
    public boolean isValidName(String name){return name!=null&&name.length()>=3&&name.length()<=5&&name.matches("[a-zA-Z0-9_]+");}
    public boolean create(Player leader,String name){if(leader==null||!isValidName(name)||get(leader)!=null||getByName(name)!=null)return false;String id=UUID.randomUUID().toString().substring(0,8);Set<UUID> m=new HashSet<>();m.add(leader.getUniqueId());Team t=new Team(id,name,leader.getUniqueId(),m,null,false);teams.put(id,t);byPlayer.put(leader.getUniqueId(),id);save();return true;}
    ...
    public boolean invite(Player from,Player to){Team t=get(from);if(t==null||to==null||get(to)!=null||t.members().size()>=10)return false;invites.put(to.getUniqueId(),t.id());Bukkit.getScheduler().runTaskLater(plugin,()->{String id=invites.get(to.getUniqueId());if(t.id().equals(id))invites.remove(to.getUniqueId());},20L*60);return true;}
    public boolean accept(Player p){String id=invites.remove(p.getUniqueId());if(id==null||get(p)!=null)return false;Team t=teams.get(id);if(t==null||t.members().size()>=10)return false;t.members().add(p.getUniqueId());byPlayer.put(p.getUniqueId(),id);save();return true;}
    ...
    public void sendTeamChat(Player sender,String message){Team t=get(sender);if(t==null)return;for(UUID u:t.members()){Player p=Bukkit.getPlayer(u);if(p!=null)p.sendMessage(ColorUtil.text("&b[Takım] &f"+sender.getName()+" &8» &f"+message));}}
}
